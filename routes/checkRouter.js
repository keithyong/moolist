...
router.post('/check/:id', (req, res) => {
    pg.connect(config.pg_connection_string, (err, client, done) => {
        if (err) {
            ...
            client.query('UPDATE todo SET completion_time = (CASE completed WHEN false THEN now() ELSE null END), completed = NOT completed WHERE todo.id=$1;', [req.params.id], (err, result) => {
                if (err) {
                    pgErrHandler(res, err)
                    client.end()
                } else {
